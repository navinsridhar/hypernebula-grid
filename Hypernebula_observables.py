...
def run_radio(Mdot_w_f, t_active_f, v_w_f, v_j_f, epsilon_e_f, sigma_f, eta_f):
    ...
    if alpha==0.0:
        ...
        eps=5e-4
        t = t0*(1.0+10**np.linspace(-2,7,1000)) #t0*(1.0+10**np.linspace(-3,2.3,1e3)) 
        #uno edit t = t0*(1.0+10**np.linspace(-3,7.3,1000))
    elif alpha>1.0:
    ...
    outputfilename = f"./models/output_Mdot_{Mdot_w_str}_tact_{int(t_active_f)}_vw_{v_w_f}_vj_{v_j_f}_epsilon_e_{epsilon_e_f}_sigma_{sigma_f}_eta_{eta_f}".replace('.','').replace('+','')
